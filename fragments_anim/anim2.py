...
def faille(zname,z,thick,strength):
    ...
    # draw wipe: reveal = clamp((y - (1-progress))*8)
    prog=n.nodes.new("ShaderNodeValue"); prog.label="prog"
    one_m=n.nodes.new("ShaderNodeMath"); one_m.operation='SUBTRACT'; one_m.inputs[0].default_value=1.0
    n.links.new(prog.outputs[0],one_m.inputs[1])             # 1 - progress
    ysub=n.nodes.new("ShaderNodeMath"); ysub.operation='SUBTRACT'
    n.links.new(sep.outputs["Y"],ysub.inputs[0]); n.links.new(one_m.outputs[0],ysub.inputs[1])  # y-(1-prog)
    ...
    ga=n.nodes.new("ShaderNodeValue"); ga.label="galpha"
    ...
    # keyframes: progress draws f8->26, global fade-out f44->58
...
faille("glow",0.04,0.050,3.0)
faille("core",0.05,0.007,11.0)
...
keyloc(0,[(1,0.0),(15,0.035),(30,0.0),(45,-0.035),(60,0.0)])
keyloc(1,[(1,0.0),(30,0.022),(60,0.0)])
...
except Exception as ex: print("tf",ex)
...
scene.render.image_settings.file_format='PNG'; scene.render.filepath=out_dir+"\\"+name+"_"
bpy.ops.render.render(animation=True); print("ANIM_DONE",name)
